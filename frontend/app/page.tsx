  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-[#f8fafc] pb-24 font-sans text-slate-900 animate-in fade-in select-none">
      ...
      <div className="p-4 space-y-6">
        ...
        {/* 6. LIVE ACTIVE DELIVERIES TRACKING SECTION */}
        <div className="space-y-3">
          ...
          {activeDeliveries.length === 0 ? (
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 text-center space-y-3">
              <div className="w-12 h-12 rounded-2xl bg-blue-50 text-[#002b5c] flex items-center justify-center mx-auto font-bold">
                <Package className="w-6 h-6" />
              </div>
              <div>
                <h3 className="text-sm font-extrabold text-slate-800">No Active Deliveries</h3>
                <p className="text-xs text-slate-500 font-medium mt-1">Send a parcel or post a trip to start real-time tracking.</p>
              </div>
              <div className="pt-2 flex justify-center gap-3">
                <Link href="/send" className="bg-[#002b5c] text-white text-xs font-bold px-4 py-2 rounded-xl hover:bg-[#001f44] transition">
                  Send Parcel
                </Link>
                <Link href="/post-trip" className="bg-slate-100 text-slate-700 text-xs font-bold px-4 py-2 rounded-xl hover:bg-slate-200 transition">
                  Post Trip
                </Link>
              </div>
            </div>
            ...
            activeDeliveries.map((delivery) => (
              <div key={delivery.id} className="bg-white rounded-3xl p-4 shadow-sm border border-slate-100 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <span className="text-[10px] font-bold text-slate-400 block leading-tight">{delivery.id}</span>
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-500 pt-2 border-t border-slate-100">
                  <div className="flex items-center gap-1 font-medium">
                    ...
                    <span>{delivery.expected_delivery_time || 'Express Delivery'}</span>
                  </div>
                  <Link href={`/deliveries/${delivery.id}`} className="flex items-center gap-1 font-bold text-[#002b5c] hover:underline">
                    <UserIcon className="w-3.5 h-3.5 text-slate-400" />
                    <span>{delivery.traveler?.full_name || delivery.sender?.full_name || 'Track Live'}</span>
